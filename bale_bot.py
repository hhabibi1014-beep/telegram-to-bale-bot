...
def start(message):
    bot.send_message(message.chat.id, "سلام! من آماده‌ام.", reply_markup=get_main_keyboard(message.chat.id))
...
def handle_bale(message):
    ...
    if message.text == "🔄 بازبینی ۵۰ پیام اخیر (فقط مدیر)":
        ...
        try:
            ...
            bot.reply_to(message, "مشکلی در ارتباط با هوش مصنوعی رخ داد.")
